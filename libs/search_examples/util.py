...
#        delete_words = ['____','DOCUMENT OF INTERNATIONAL MONETARY FUND','Download Date']
        replace_regex = re.compile(r'\uf0b7|\x0c|____|DOCUMENT OF INTERNATIONAL MONETARY FUND|Download Date')
...
def construct_rex(keywords):
    r_keywords = [r'\b' + re.escape(k.strip()) + r'(s|es|ly|ing)?\b' for k in keywords]    # tronsform keyWords list to a patten list, find both s and es 
...
def construct_rex2(keywords):
    ## allow inputs to be seperated by , - mean all words need to be in the context
    r_keywords = list()
    for k in keywords:
        if "," in k:
            kl = k.split(',')
            kl = [i.strip() for i in kl]
            kl = '.*?'.join(kl)
            rk = r'\b'+kl+r'(s|es|ly|ing)?\b'
        else:
            rk = r'\b' + re.escape(k.strip()) + r'(s|es|ly|ing)?\b'
        
        r_keywords.append(rk)
    rex = re.compile('|'.join(r_keywords),flags=re.I)                        # use or to join all of them, ignore casing
    #match = [(m.start(),m.group()) for m in rex.finditer(content)]          # get the position and the word
    return rex
...
def find_exact_keywords_with_pos(content,keywords,rex=None):
    if rex is None: 
        rex = construct_rex(keywords)
    content = content.replace('\n', '').replace('\r', '')#.replace('.',' .')
    match = [(m.group(),m.start()) for m in rex.finditer(content)]  
    return match
